...
namespace pad.core.serialization
{
    ...
    public static class AmigaExecutable
    {
		...
		public static void Write(IDataWriter dataWriter, List<IHunk> hunks)
		{
			...
			foreach (var hunk in realHunks)
			{
				if ((hunk.MemoryFlags & 0x3fffffff) != 0 ||
					...
					throw new WorkflowException("Invalid hunk memory flags");

				if (hunk.Size > 0x3fffffff)
					throw new WorkflowException("Invalid hunk size");
			...
			foreach (var hunk in hunks)
			{
				...
				switch (hunk.Type)
                {
					...
					case HunkType.Data:
                    {
						...
						if (dataHunk.Data is null)
							throw new NullReferenceException("Data of code/data hunk was null.");

						if (dataHunk.Data.Length != dataHunk.NumEntries * 4)
							throw new InvalidDataException("Mismatching NumEntries value and Data length for code/data hunk.");

						if (dataHunk.NumEntries != hunk.Size)
							throw new InvalidDataException("Mismatching NumEntries value and hunk size for code/data hunk.");
					...
					case HunkType.BSS:
                    {
						...
						if (bssHunk.NumEntries != hunk.Size)
							throw new InvalidDataException("Mismatching NumEntries value and hunk size for BSS hunk.");
					...
					case HunkType.Reloc32:
                    {
						...
						if (dataWriter.Position - start != hunk.Size)
							throw new WorkflowException("Error writing RELOC32 hunk data");
		...
		public static List<IHunk> Read(IDataReader dataReader, bool deplodeIfNecessary = true)
        {
			...
			static void Throw() => throw new InvalidDataException("Invalid executable file.");
			...
			for (int i = 0; i < numHunks; ++i)
			{
				...
				switch (type)
				{
					...
					default:
						throw new NotSupportedException($"Unsupported hunk type: {type}.");
				}
			...
			if (deplodeIfNecessary)
			{
				...
				{
					imploded = true;
					var firstHunkData = ((Hunk)hunks.First()).Data ?? throw new InvalidDataException("First hunk data is null.");
					...
					if (!imploded)
					{
						...
						var start = Encoding.GetString(firstHunkData.Take(200).ToArray());
		...
		static List<IHunk> ReadImploded(List<IHunk> imploderHunks)
        {
			...
			while (true)
			{
				...
				// TODO: I guess the hunk types are directly encoded somewhere as the flags are only the memory flags.
				...
				if (flags == 2 || flags == 3) // BSS or DATA
                {
					if (reader.Position < reader.Size && (reader.PeekDword() & 0x3fffffff) != 0) // a size follows -> no BSS but DATA
					{
						...
						if (hunkSize * 4 != hunkSizes[hunkSizeIndex])
							throw new InvalidDataException("Invalid hunk data size.");
        ...
        {
			deplodedHunkSizes = new();
			deplodedMemFlags = new();
			...
			Buffer.BlockCopy(lastCodeHunk.Data ?? throw new InvalidDataException("Last imploded code hunk has no data."), 0x188, table, 0, table.Length);
			...
			var data = dataHunk.Data ?? throw new InvalidDataException("Imploded data hunk has no data.");
			...
			fixed (byte* ptr = &data[0])
			{
				if (!Deploder.Deplode(ptr, buffer, table, dataSize, firstLiteralLength, initialBitBuffer))
					throw new InvalidDataException("Invalid imploded data.");
			}
