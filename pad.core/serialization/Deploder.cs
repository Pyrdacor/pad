...
namespace pad.core.serialization
{
    public static class Deploder
    {
		...
		public static unsafe byte[] DeplodeFimp(IDataReader dataReader)
        {
			if (dataReader.PeekDword() != 0x494d5021) // "IMP!"
				throw new InvalidDataException("No valid IMP data");
			...
			fixed (byte* ptr = preparedData)
			{
				if (!Deplode(ptr, destBuffer, table, (uint)implodedSize, firstLiteralLength, initialBitBuffer))
					throw new Exception("Error exploding data");
			}

			if (destBuffer.Size != explodedSize)
				throw new Exception("Exploded size does not match the value in the header");
